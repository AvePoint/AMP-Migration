...
function Restart-SPJob
{
    ...
    param(    
        ...
        [Parameter(Mandatory = $true, ParameterSetName = 'SPJob_Restart')]
        [Microsoft.PowerShell.AMPMigration.v001.Models.JobExecutionModel]
        $Settings
    )
...
'@ 
    ...
    if ('SPJob_Restart' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation RestartWithHttpMessagesAsync on $AvePointMigrationPlatformAPI.'
        $taskResult = $AvePointMigrationPlatformAPI.SPJob.RestartWithHttpMessagesAsync($Id, $Settings)
    } else {
